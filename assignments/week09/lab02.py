...
def get_range_hint(number, current_min=1, current_max=100):
    mins = number - 12
    maxs = number + 12
    return f"The number is between {current_min} - {current_max} or {maxs} - {mins}"
...
def get_thefirst_digit_hint(number):
    ...
    text = str(number)
    return f"The first digit of this number is {number//10},{text[0]}"
    pass
...
while True :
    guess_number = int(input(f"Attempt {i} - Enter your guess: "))
    ...
    if i == 3:
        ...
        print(get_range_hint(random_number,min,max))
    elif i == 10:
